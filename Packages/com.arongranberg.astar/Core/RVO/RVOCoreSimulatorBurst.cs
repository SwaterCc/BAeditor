...
namespace Pathfinding.RVO
{
    ...
    public class SimulatorBurst
    {
        ...
        struct Agent : IAgent
        {
            ...
            {
                get => simulator.simulationData.position[AgentIndex];
                set => simulator.simulationData.position[AgentIndex] = value;
            }

            public bool Locked
            {
                get => simulator.simulationData.locked[AgentIndex];
                set => simulator.simulationData.locked[AgentIndex] = value;
            }

            public float Radius
            {
                get => simulator.simulationData.radius[AgentIndex];
                set => simulator.simulationData.radius[AgentIndex] = value;
            }

            public float Height
            {
                get => simulator.simulationData.height[AgentIndex];
                set => simulator.simulationData.height[AgentIndex] = value;
            }

            public float AgentTimeHorizon
            {
                get => simulator.simulationData.agentTimeHorizon[AgentIndex];
                set => simulator.simulationData.agentTimeHorizon[AgentIndex] = value;
            }

            public float ObstacleTimeHorizon
            {
                get => simulator.simulationData.obstacleTimeHorizon[AgentIndex];
                set => simulator.simulationData.obstacleTimeHorizon[AgentIndex] = value;
            }

            public int MaxNeighbours
            {
                get => simulator.simulationData.maxNeighbours[AgentIndex];
                set => simulator.simulationData.maxNeighbours[AgentIndex] = value;
            }

            public RVOLayer Layer
            {
                get => simulator.simulationData.layer[AgentIndex];
                set => simulator.simulationData.layer[AgentIndex] = value;
            }

            public RVOLayer CollidesWith
            {
                get => simulator.simulationData.collidesWith[AgentIndex];
                set => simulator.simulationData.collidesWith[AgentIndex] = value;
            }

            public float FlowFollowingStrength
            {
                get => simulator.simulationData.flowFollowingStrength[AgentIndex];
                set => simulator.simulationData.flowFollowingStrength[AgentIndex] = value;
            }

            public AgentDebugFlags DebugFlags
            {
                get => simulator.simulationData.debugFlags[AgentIndex];
                set => simulator.simulationData.debugFlags[AgentIndex] = value;
            }

            public float Priority
            {
                get => simulator.simulationData.priority[AgentIndex];
                set => simulator.simulationData.priority[AgentIndex] = value;
            }

            public int HierarchicalNodeIndex
            {
                get => simulator.simulationData.hierarchicalNodeIndex[AgentIndex];
                set => simulator.simulationData.hierarchicalNodeIndex[AgentIndex] = value;
            }

            public SimpleMovementPlane MovementPlane
            {
                get => new SimpleMovementPlane(simulator.simulationData.movementPlane[AgentIndex].rotation);
                set => simulator.simulationData.movementPlane[AgentIndex] = new NativeMovementPlane(value);
            }

            public Action PreCalculationCallback
            {
                set => simulator.agentPreCalculationCallbacks[AgentIndex] = value;
            }

            public Action DestroyedCallback
            {
                set => simulator.agentDestroyCallbacks[AgentIndex] = value;
            }
            ...
            public bool AvoidingAnyAgents
            {
                ...
                {
                    simulator.BlockUntilSimulationStepDone();
                    return simulator.outputData.blockedByAgents[AgentIndex * SimulatorBurst.MaxBlockingAgentCount] !=
                           -1;
                }
            }
            ...
            public void SetObstacleQuery(GraphNode sourceNode)
            {
                HierarchicalNodeIndex = sourceNode != null && !sourceNode.Destroyed && sourceNode.Walkable
                    ? sourceNode.HierarchicalNodeIndex
                    : -1;
            }
            ...
            public void ForceSetVelocity(Vector3 velocity)
            {
                ...
                simulator.simulationData.targetPoint[AgentIndex] =
                    simulator.simulationData.position[AgentIndex] + (float3)velocity * 1000;
        ...
        public struct AgentData
        {
            ...
            public void Realloc(int size, Allocator allocator)
            {
                ...
                Util.Memory.Realloc(ref allowedVelocityDeviationAngles, size, allocator,
            ...
            public bool HasDebugFlag(int agentIndex, AgentDebugFlags flag) =>
                Unity.Burst.CompilerServices.Hint.Unlikely((debugFlags[agentIndex] & flag) != 0);
        ...
        public struct AgentOutputData
        {
            ...
            public void Realloc(int size, Allocator allocator)
            {
                ...
                Util.Memory.Realloc(ref blockedByAgents, size * MaxBlockingAgentCount, allocator,
                    NativeArrayOptions.UninitializedMemory);
                Util.Memory.Realloc(ref effectivelyReachedDestination, size, allocator,
                    NativeArrayOptions.UninitializedMemory);
            ...
            public void Move(int fromIndex, int toIndex)
            {
                ...
                {
                    blockedByAgents[toIndex * MaxBlockingAgentCount + i] =
                        blockedByAgents[fromIndex * MaxBlockingAgentCount + i];
                }
        ...
        public struct TemporaryAgentData
        {
            ...
            public void Realloc(int size, Allocator allocator)
            {
                Util.Memory.Realloc(ref desiredTargetPointInVelocitySpace, size, allocator,
                    NativeArrayOptions.UninitializedMemory);
                ...
                Util.Memory.Realloc(ref collisionVelocityOffsets, size, allocator,
                    NativeArrayOptions.UninitializedMemory);
                Util.Memory.Realloc(ref neighbours, size * MaxNeighbourCount, allocator,
                    NativeArrayOptions.UninitializedMemory);
            }
        ...
        public SimulatorBurst(MovementPlane movementPlane)
        {
            ...
            quadtree.BuildJob(simulationData.position, simulationData.version, simulationData.desiredSpeed,
                simulationData.radius, 0, movementPlane).Run();
        }
        ...
        public AgentIndex AddAgentBurst(float3 position)
        {
            ...
            simulationData.movementPlane[agentIndex] = new NativeMovementPlane((movementPlane == MovementPlane.XY
                ? SimpleMovementPlane.XYPlane
                : SimpleMovementPlane.XZPlane).rotation);
        ...
        {
            throw new System.NotImplementedException(
                "Use AddAgent(position) instead. Agents are not persistent after being removed.");
        }
        ...
        public void RemoveAgent(AgentIndex agent)
        {
            ...
            if (!AgentExists(agent))
                throw new System.InvalidOperationException("Trying to remove agent which does not exist");
        ...
        public JobHandle Update(JobHandle dependency, float dt, bool drawGizmos, Allocator allocator)
        {
            ...
            else if (movementPlane == MovementPlane.XZ)
        ...
        JobHandle UpdateInternal<T>(JobHandle dependency, float deltaTime, bool drawGizmos, Allocator allocator)
            where T : struct, IMovementPlaneWrapper
        {
            ...
            var quadtreeJob = quadtree.BuildJob(simulationData.position, simulationData.version, outputData.speed,
                simulationData.radius, numAgents, movementPlane).Schedule(dependency);
            ...
            combinedJob =
                JobHandle.CombineDependencies(horizonJob2, hardCollisionsJob1, navmeshEdgeDataLock.dependency);
