...
namespace Pathfinding.RVO
{
    ...
    {
        [ReadOnly] public SimulatorBurst.AgentData agentData;

        [ReadOnly] public SimulatorBurst.AgentOutputData previousOutput;

        [WriteOnly] public SimulatorBurst.TemporaryAgentData temporaryAgentData;
        ...
        public void Execute()
        {
            for (int i = startIndex; i < endIndex; i++)
            {
                ...
                {
                    var desiredTargetPointInVelocitySpace = agentData.movementPlane[i]
                        .ToPlane(agentData.targetPoint[i] - agentData.position[i]);
                    ...
                    var currentVelocity = math.normalizesafe(previousOutput.targetPoint[i] - agentData.position[i]) *
                                          previousOutput.speed[i];

                    // Calculate the desired velocity from the point we want to reach
                    temporaryAgentData.desiredVelocity[i] = agentData.movementPlane[i]
                        .ToWorld(math.normalizesafe(desiredTargetPointInVelocitySpace) * agentData.desiredSpeed[i], 0);
    ...
    public struct JobHorizonAvoidancePhase1 : Pathfinding.Jobs.IJobParallelForBatched
    {
        ...
        [ReadOnly] public NativeArray<float2> desiredTargetPointInVelocitySpace;
        ...
        public void Execute(int startIndex, int count)
        {
            ...
            for (int i = startIndex; i < startIndex + count; i++)
            {
                ...
                float desiredAngle = math.atan2(desiredTargetPointInVelocitySpace[i].y,
                    desiredTargetPointInVelocitySpace[i].x);
                ...
                var agentNeighbours =
                    neighbours.Slice(i * SimulatorBurst.MaxNeighbourCount, SimulatorBurst.MaxNeighbourCount);
                ...
                for (; firstPositiveIndex < eventCount; firstPositiveIndex++)
                    if (angles[firstPositiveIndex] > 0)
                        break;
    ...
    {
        [ReadOnly] public NativeArray<int> neighbours;
        [ReadOnly] public NativeArray<AgentIndex> versions;
        public NativeArray<float3> desiredVelocity;
        public NativeArray<float2> desiredTargetPointInVelocitySpace;

        [ReadOnly] public NativeArray<NativeMovementPlane> movementPlane;
        ...
        public void Execute(int startIndex, int count)
        {
            for (int i = startIndex; i < startIndex + count; i++)
            {
                ...
                {
                    float sum = 0;
                    var agentNeighbours = neighbours.Slice(i * SimulatorBurst.MaxNeighbourCount,
                        SimulatorBurst.MaxNeighbourCount);
                    for (int j = 0; j < agentNeighbours.Length && agentNeighbours[j] != -1; j++)
                    {
                        var other = agentNeighbours[j];
                        var otherHorizonBias = -(horizonAgentData.horizonMinAngle[other] +
                                                 horizonAgentData.horizonMaxAngle[other]);
                ...
                float bestAngle = horizonAgentData.horizonSide[i] < 0
                    ? horizonAgentData.horizonMinAngle[i]
                    : horizonAgentData.horizonMaxAngle[i];
                ...
                desiredTargetPointInVelocitySpace[i] =
                    math.length(desiredTargetPointInVelocitySpace[i]) * desiredDirection;
            }
        }
    }

    [BurstCompile(FloatMode = FloatMode.Fast)]
    public struct JobHardCollisions<MovementPlaneWrapper> : Pathfinding.Jobs.IJobParallelForBatched
        where MovementPlaneWrapper : struct, IMovementPlaneWrapper
    {
        [ReadOnly] public SimulatorBurst.AgentData agentData;
        [ReadOnly] public NativeArray<int> neighbours;
        [WriteOnly] public NativeArray<float2> collisionVelocityOffsets;
    ...
    [BurstCompile(CompileSynchronously = false, FloatMode = FloatMode.Fast)]
    public struct JobRVOCalculateNeighbours<MovementPlaneWrapper> : Pathfinding.Jobs.IJobParallelForBatched
        where MovementPlaneWrapper : struct, IMovementPlaneWrapper
    {
        [ReadOnly] public SimulatorBurst.AgentData agentData;

        [ReadOnly] public RVOQuadtreeBurst quadtree;

        public NativeArray<int> outNeighbours;

        [WriteOnly] public SimulatorBurst.AgentOutputData output;
        ...
        public void Execute(int startIndex, int count)
        {
            NativeArray<float> neighbourDistances = new NativeArray<float>(SimulatorBurst.MaxNeighbourCount,
        ...
        void CalculateNeighbours(int agentIndex, NativeArray<int> neighbours, NativeArray<float> neighbourDistances)
        {
            ...
            int numNeighbours = 0;
            while (numNeighbours < maxNeighbourCount && math.isfinite(neighbourDistances[numNeighbours]))
                numNeighbours++;
            ...
            for (int i = 0; i < numNeighbours; i++)
            {
                ...
                float maxY = math.min(localElevation + agentData.height[agentIndex],
                    otherElevation + agentData.height[otherIndex]);
                ...
                if ((maxY < minY) | (otherIndex == agentIndex) |
                    (((int)agentData.collidesWith[agentIndex] & (int)agentData.layer[otherIndex]) == 0))
                {
                    numNeighbours--;
    ...
    /// </summary>
    [BurstCompile(CompileSynchronously = false, FloatMode = FloatMode.Fast)]
    public struct JobDestinationReached<MovementPlaneWrapper> : IJob
        where MovementPlaneWrapper : struct, IMovementPlaneWrapper
    {
        [ReadOnly] public SimulatorBurst.AgentData agentData;

        [ReadOnly] public SimulatorBurst.TemporaryAgentData temporaryAgentData;

        [ReadOnly] public SimulatorBurst.ObstacleData obstacleData;
        ...
        public void Execute()
        {
            ...
            var inArrows = new NativeArray<int>(agentData.position.Length * SimulatorBurst.MaxBlockingAgentCount,
                Allocator.Temp, NativeArrayOptions.UninitializedMemory);
            // Number of agents that each agent blocks
            var inArrowCounts =
                new NativeArray<int>(agentData.position.Length, Allocator.Temp, NativeArrayOptions.ClearMemory);
            ...
            var tempData =
                new NativeArray<TempAgentData>(numAgents, Allocator.Temp, NativeArrayOptions.UninitializedMemory);
            MarkerAlloc.End();
            MarkerInvert.Begin();
            ...
            for (int agentIndex = 0; agentIndex < numAgents; agentIndex++)
            {
                ...
                {
                    var blockingAgentIndex =
                        output.blockedByAgents[agentIndex * SimulatorBurst.MaxBlockingAgentCount + i];
            ...
            for (int agentIndex = 0; agentIndex < numAgents; agentIndex++)
            {
                ...
                var distToEndSq =
                    math.lengthsq(
                        movementPlane.ToPlane(ourEndOfPath - position, out float endOfPathElevationDifference));
                ...
                // Heuristic 2
                if (distToEndSq < ourRadius * ourRadius * (0.5f * 0.5f) && endOfPathElevationDifference < ourHeight &&
                    endOfPathElevationDifference > -ourHeight * 0.5f)
                {
                    reachedEndOfPath = true;
                }

                var closeToBlocked = forwardClearance < ourRadius * 0.5f;
                var slowish = ourSpeed * ourSpeed < math.max(0.01f * 0.01f,
                    math.lengthsq(temporaryAgentData.desiredVelocity[agentIndex]) * 0.25f);
                ...
                for (int i = 0; i < SimulatorBurst.MaxBlockingAgentCount; i++)
                {
                    ...
                    var circleRadius =
                        (math.sqrt(distBetweenAgentsSq) + ourRadius + agentData.radius[blockingAgentIndex]) * 0.5f;
                    var endWithinCircle =
                        math.lengthsq(movementPlane.ToPlane(ourEndOfPath - 0.5f * (position + otherPosition))) <
                ...
                var effectivelyReached = reachedEndOfPath
                    ? ReachedEndOfPath.Reached
                    : (flowFollowing ? ReachedEndOfPath.ReachedSoon : ReachedEndOfPath.NotReached);
            ...
            while (que.Length > 0)
            {
                ...
                for (int i = 0; i < SimulatorBurst.MaxBlockingAgentCount; i++)
                {
                    ...
                    var endOfPathsOverlapping =
                        math.lengthsq(otherEndOfPath - ourEndOfPath) <= distToEndSq * (0.5f * 0.5f);
                    var otherReached = output.effectivelyReachedDestination[blockingAgentIndex] ==
                                       ReachedEndOfPath.Reached;

                    if (otherReached && (endOfPathsOverlapping ||
                                         math.lengthsq(ourEndOfPath - agentData.position[blockingAgentIndex]) <
                                         math.lengthsq(ourRadius + otherRadius)))
                    {
                ...
                effectivelyReached = (ReachedEndOfPath)math.max((int)effectivelyReached,
                    (int)output.effectivelyReachedDestination[agentIndex]);
    ...
    public struct JobRVO<MovementPlaneWrapper> : Pathfinding.Jobs.IJobParallelForBatched
        where MovementPlaneWrapper : struct, IMovementPlaneWrapper
    {
        [ReadOnly] public SimulatorBurst.AgentData agentData;
        ...
        [ReadOnly] public NavmeshEdges.NavmeshBorderData navmeshEdgeData;
        ...
        void GenerateObstacleVOs(int agentIndex, NativeList<int> adjacentObstacleIdsScratch,
            NativeArray<int2> adjacentObstacleVerticesScratch, NativeArray<float> segmentDistancesScratch,
            NativeArray<int> sortedVerticesScratch, NativeArray<ORCALine> orcaLines, NativeArray<int> orcaLineToAgent,
            [NoAlias] ref int numLines, [NoAlias] in MovementPlaneWrapper movementPlane, float2 optimalVelocity)
        {
            ...
            var size = (obstacleRadius + agentRadius +
                        agentData.obstacleTimeHorizon[agentIndex] * agentData.maxSpeed[agentIndex]) *
                       new float3(2, 0, 2);
            ...
            for (int oi = 0; oi < adjacentObstacleIdsScratch.Length; oi++)
            {
                ...
                var vertices =
                    navmeshEdgeData.obstacleData.obstacleVertices.GetSpan(obstacleAllocations.verticesAllocation);
                var groups =
                    navmeshEdgeData.obstacleData.obstacleVertexGroups.GetSpan(obstacleAllocations.groupsAllocation);
                int vertexOffset = 0;
                ...
                for (int i = 0; i < groups.Length; i++)
                {
                    ...
                    adjacentObstacleVerticesScratch[startVertex] = new int2(
                        group.type == ObstacleType.Loop ? endVertex : startVertex,
                        adjacentObstacleVerticesScratch[startVertex].y);
                    adjacentObstacleVerticesScratch[endVertex] = new int2(adjacentObstacleVerticesScratch[endVertex].x,
                        group.type == ObstacleType.Loop ? startVertex : endVertex);
                ...
                for (int i = 0; i < candidateVertexCount; i++)
                {
                    ...
                    if (math.max(e1, e2) + agentHeight < agentElevation ||
                        math.min(e1, e2) > agentElevation + agentHeight)
                    {
                    ...
                    for (var j = 0; j < numLines; j++)
                    {
                        ...
                        if (
                            ...
                            det(inverseObstacleTimeHorizon * v1Position - line.point, line.direction) -
                            inverseObstacleTimeHorizon * obstacleRadius >= -EPSILON &&
                            det(inverseObstacleTimeHorizon * v2Position - line.point, line.direction) -
                            inverseObstacleTimeHorizon * obstacleRadius >= -EPSILON
                        )
                        {
                            alreadyCovered = true;
                    ...
                    var distanceToSegmentSq =
                        math.lengthsq((v1Position + math.clamp(distanceAlongSegment, 0, length) * segmentDir));
                    ...
                    if ((distanceAlongSegment < 0 || distanceAlongSegment > 1) &&
                        distanceToLineSq <= obstacleRadius * obstacleRadius)
                    {
                    ...
                    var t = cutoffLength <= 0.00001f
                        ? 0.5f
                        : math.dot(optimalVelocity - leftCutoff, cutoffDir) / cutoffLength;
                    ...
                    if ((t < 0.0f && tLeft < 0.0f) || (t > 1.0f && tRight < 0.0f) ||
                        (cutoffLength <= 0.00001f && tLeft < 0.0f && tRight < 0.0f))
                    {
                    ...
                    float distToCutoff = (t > 1.0f || t < 0.0f || cutoffLength < 0.0001f
                        ? math.INFINITY
                        : math.lengthsq(optimalVelocity - (leftCutoff + t * cutoffDir)));
                    float distToLeftLeg = (tLeft < 0.0f
                        ? math.INFINITY
                        : math.lengthsq(optimalVelocity - (leftCutoff + tLeft * leftLegDirection)));
                    float distToRightLeg = (tRight < 0.0f
                        ? math.INFINITY
                        : math.lengthsq(optimalVelocity - (rightCutoff + tRight * rightLegDirection)));
                    var selected = 0;
                    var mn = distToCutoff;
                    if (distToLeftLeg < mn)
                    {
                    ...
                    if (selected == 0)
                    {
                        ...
                        orcaLines[numLines++] = new ORCALine
                        {
                            point = leftCutoff + obstacleRadius * inverseObstacleTimeHorizon *
                                new float2(segmentDir.y, -segmentDir.x),
                            direction = -segmentDir,
                        };
                    }
                    else if (selected == 1)
                    {
                        if (!isLeftLegForeign)
                        {
                            ...
                            {
                                point = leftCutoff + obstacleRadius * inverseObstacleTimeHorizon *
                                    new float2(-leftLegDirection.y, leftLegDirection.x),
                    ...
                    else if (selected == 2)
                    {
                        if (!isRightLegForeign)
                        {
                            ...
                            orcaLines[numLines++] = new ORCALine
                            {
                                point = rightCutoff + obstacleRadius * inverseObstacleTimeHorizon *
                                    new float2(rightLegDirection.y, -rightLegDirection.x),
        ...
        public void ExecuteORCA(int startIndex, int batchSize)
        {
            ...
            NativeArray<ORCALine> orcaLines = new NativeArray<ORCALine>(
                SimulatorBurst.MaxNeighbourCount + MaxObstacleCount, Allocator.Temp,
                NativeArrayOptions.UninitializedMemory);
            NativeArray<ORCALine> scratchBuffer = new NativeArray<ORCALine>(
                SimulatorBurst.MaxNeighbourCount + MaxObstacleCount, Allocator.Temp,
                NativeArrayOptions.UninitializedMemory);
            NativeArray<float> segmentDistancesScratch = new NativeArray<float>(SimulatorBurst.MaxObstacleVertices,
                Allocator.Temp, NativeArrayOptions.UninitializedMemory);
            NativeArray<int> sortedVerticesScratch = new NativeArray<int>(SimulatorBurst.MaxObstacleVertices,
                Allocator.Temp, NativeArrayOptions.UninitializedMemory);
            NativeArray<int2> adjacentObstacleVertices = new NativeArray<int2>(4 * SimulatorBurst.MaxObstacleVertices,
                Allocator.Temp, NativeArrayOptions.UninitializedMemory);
            NativeArray<int> orcaLineToAgent = new NativeArray<int>(SimulatorBurst.MaxNeighbourCount + MaxObstacleCount,
                Allocator.Temp, NativeArrayOptions.UninitializedMemory);
            ...
            for (int agentIndex = startIndex; agentIndex < endIndex; agentIndex++)
            {
                ...
                GenerateObstacleVOs(agentIndex, adjacentObstacleIdsScratch, adjacentObstacleVertices,
                    segmentDistancesScratch, sortedVerticesScratch, orcaLines, orcaLineToAgent, ref numLines,
                    in movementPlane, optimalVelocity);
                int numFixedLines = numLines;

                var neighbours = temporaryAgentData.neighbours.Slice(agentIndex * SimulatorBurst.MaxNeighbourCount,
                    SimulatorBurst.MaxNeighbourCount);
                ...
                for (int neighbourIndex = 0; neighbourIndex < neighbours.Length; neighbourIndex++)
                {
                    ...
                    float2 otherOptimalVelocity = movementPlane.ToPlane(math.lerp(
                        temporaryAgentData.currentVelocity[otherIndex], temporaryAgentData.desiredVelocity[otherIndex],
                        math.clamp(2 * avoidanceStrength - 1, 0, 1)));
                    ...
                    if (agentData.flowFollowingStrength[otherIndex] > 0)
                    {
                        ...
                        var strength = agentData.flowFollowingStrength[otherIndex] *
                                       agentData.flowFollowingStrength[agentIndex];
                        var relativeDir = math.normalizesafe(relativePosition);
                        otherOptimalVelocity -= relativeDir *
                                                (strength * math.min(0, math.dot(otherOptimalVelocity, relativeDir)));
                    }
                    ...
                    var minimumTimeToCollision = math.max(0, dist - combinedRadius) / math.max(combinedRadius,
                        agentData.desiredSpeed[agentIndex] + agentData.desiredSpeed[otherIndex]);
                    ...
                    var tempInverseTimeHorizon = 1.0f / math.max(0.1f * agentTimeHorizon,
                        agentTimeHorizon * math.clamp(math.sqrt(2f * minimumTimeToCollision), 0, 1));

                    orcaLines[numLines] = new ORCALine(localPosition, relativePosition, optimalVelocity,
                        otherOptimalVelocity, combinedRadius, 0.1f, tempInverseTimeHorizon);
                    ...
                    if (agentData.HasDebugFlag(agentIndex, AgentDebugFlags.AgentVOs))
                    {
                        ...
                        DrawVO(draw, relativePosition * tempInverseTimeHorizon + otherOptimalVelocity,
                            combinedRadius * tempInverseTimeHorizon, otherOptimalVelocity, Color.black);
                        draw.PopMatrix();
                    }
#endif
                }
                ...
                var desiredTargetPointInVelocitySpace =
                    temporaryAgentData.desiredTargetPointInVelocitySpace[agentIndex];
                ...
                bool insideAnyVO =
                    BiasDesiredVelocity(orcaLines.AsUnsafeSpan().Slice(numFixedLines, numLines - numFixedLines),
                        ref desiredVelocity, ref desiredTargetPointInVelocitySpace, symmetryBias);
                // If the velocity is outside all agent orca half-planes, do a more thorough check of all orca lines (including obstacles).
                insideAnyVO = insideAnyVO ||
                              DistanceInsideVOs(orcaLines.AsUnsafeSpan().Slice(0, numLines), desiredVelocity) > 0;
                ...
                if (agentData.HasDebugFlag(agentIndex, AgentDebugFlags.ObstacleVOs))
                {
                    ...
                    for (int i = 0; i < numLines; i++)
                    {
                        orcaLines[i].DrawAsHalfPlane(draw, agentData.radius[agentIndex] * 5.0f, 1.0f,
                            i >= numFixedLines ? Color.magenta : Color.Lerp(Color.magenta, Color.black, 0.5f));
                    }
                ...
                if (!insideAnyVO &&
                    math.all(math.abs(temporaryAgentData.collisionVelocityOffsets[agentIndex]) < 0.001f))
                {
                    ...
                    if (agentData.HasDebugFlag(agentIndex, AgentDebugFlags.DesiredVelocity))
                    {
                        ...
                        draw.xy.Cross(movementPlane.ToWorld(localPosition + desiredTargetPointInVelocitySpace),
                            Color.yellow);
                    }
#endif

                    output.targetPoint[agentIndex] =
                        position + movementPlane.ToWorld(desiredTargetPointInVelocitySpace, 0);
                    output.speed[agentIndex] = agentData.desiredSpeed[agentIndex];
                    output.blockedByAgents[agentIndex * SimulatorBurst.MaxBlockingAgentCount] = -1;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        desiredVelocityLeftDir = math.select(float2.zero,
                            desiredVelocityLeftDir * math.rcp(desiredVelocityLeftSegmentLength),
                            desiredVelocityLeftSegmentLength > math.FLT_MIN_NORMAL);
                        ...
                        desiredVelocityRightDir = math.select(float2.zero,
                            desiredVelocityRightDir * math.rcp(desiredVelocityRightSegmentLength),
                            desiredVelocityRightSegmentLength > math.FLT_MIN_NORMAL);

                        // var tOptimal = ClosestPointOnSegment(desiredVelocityLeft, desiredVelocityDir, desiredVelocity, 0, desiredVelocitySegmentLength);

                        var lin1 = LinearProgram2DSegment(orcaLines, numLines, maxSpeed, desiredVelocityLeft,
                            desiredVelocityLeftDir, 0, desiredVelocityLeftSegmentLength, 1.0f);
                        var lin2 = LinearProgram2DSegment(orcaLines, numLines, maxSpeed, desiredVelocityRight,
                            desiredVelocityRightDir, 0, desiredVelocityRightSegmentLength, 1.0f);
                        ...
                        {
                            lin = math.lengthsq(lin1.velocity - desiredVelocity) <
                                  math.lengthsq(lin2.velocity - desiredVelocity)
                                ? lin1
                                : lin2;
                        }
                    }
                    ...
                    if (lin.firstFailedLineIndex < numLines)
                    {
                        ...
                        LinearProgram3D(orcaLines, numLines, numFixedLines, lin.firstFailedLineIndex, maxSpeed,
                            ref newVelocity, scratchBuffer);
                    }
                    else
                    {
                        newVelocity = lin.velocity;
                    ...
                    if (agentData.HasDebugFlag(agentIndex, AgentDebugFlags.ChosenVelocity))
                    {
                        ...
                        draw.Arrow(position + movementPlane.ToWorld(desiredVelocity),
                            position + movementPlane.ToWorld(newVelocity), Color.magenta);
                    }
#endif
                    ...
                    {
                        if (orcaLineToAgent[i] != -1 &&
                            det(orcaLines[i].direction, orcaLines[i].point - newVelocity) >= -0.001f)
                        {
                            // We are blocked by this line
                            output.blockedByAgents[
                                    agentIndex * SimulatorBurst.MaxBlockingAgentCount + blockedByAgentCount] =
                                orcaLineToAgent[i];
                            blockedByAgentCount++;
                        }
                    }

                    if (blockedByAgentCount < SimulatorBurst.MaxBlockingAgentCount)
                        output.blockedByAgents
                            [agentIndex * SimulatorBurst.MaxBlockingAgentCount + blockedByAgentCount] = -1;
                    ...
                    var targetDir =
                        math.normalizesafe(movementPlane.ToPlane(agentData.targetPoint[agentIndex] - position));
                    var forwardClearance =
                        CalculateForwardClearance(neighbours, movementPlane, position, agentRadius, targetDir);
                    output.forwardClearance[agentIndex] = forwardClearance;
                    if (agentData.HasDebugFlag(agentIndex, AgentDebugFlags.ForwardClearance) &&
                        forwardClearance < float.PositiveInfinity)
                    {
        ...
        /// </summary>
        float CalculateForwardClearance(NativeSlice<int> neighbours, MovementPlaneWrapper movementPlane,
            float3 position, float radius, float2 targetDir)
        {
            ...
            for (int i = 0; i < neighbours.Length; i++)
            {
                ...
                var discriminant = combinedRadius * combinedRadius -
                                   (distToOtherSq - distToClosestPointAlongRay * distToClosestPointAlongRay);
        ...
        struct ORCALine
        {
            ...
            public void DrawAsHalfPlane(CommandBuilder draw, float halfPlaneLength, float halfPlaneWidth, Color color)
            {
                ...
                draw.SolidBox(new float3(p, 0), quaternion.RotateZ(math.atan2(direction.y, direction.x)),
                    new float3(halfPlaneLength, halfPlaneWidth, 0.01f), new Color(0, 0, 0, 0.5f));
            }

            public ORCALine(float2 position, float2 relativePosition, float2 velocity, float2 otherVelocity,
                float combinedRadius, float timeStep, float invTimeHorizon)
            {
                ...
                if (distSq > combinedRadiusSq)
                {
                    ...
                    else
                    {
                        ...
                        if (det(relativePosition, w) > 0.0f)
                        {
                            ...
                            direction = (relativePosition * legDistance +
                                         new float2(-relativePosition.y, relativePosition.x) * combinedRadius) / distSq;
                        }
                        else
                        {
        ...
        /// </summary>
        static bool BiasDesiredVelocity(UnsafeSpan<ORCALine> lines, ref float2 desiredVelocity,
            ref float2 targetPointInVelocitySpace, float maxBiasRadians)
        {
            ...
            if (desiredVelocityMagn >= 0.001f)
            {
                ...
                targetPointInVelocitySpace +=
                    new float2(targetPointInVelocitySpace.y, -targetPointInVelocitySpace.x) * angle;
            }
        ...
        static bool ClipBoundary(NativeArray<ORCALine> lines, int lineIndex, float radius, out float tLeft,
            out float tRight)
        {
        ...
        static bool LinearProgram1D(NativeArray<ORCALine> lines, int lineIndex, float radius, float2 optimalVelocity,
        ...
        static LinearProgram2Output LinearProgram2D(NativeArray<ORCALine> lines, int numLines, float radius,
            float2 optimalVelocity, bool directionOpt)
        {
            float2 result;
        ...
        /// </summary>
        static float2 ClosestSegmentSegmentPointNonIntersecting(ORCALine a, ORCALine b, float ta1, float ta2, float tb1,
            float tb2)
        {
        ...
        static LinearProgram2Output LinearProgram2DCollapsedSegment(NativeArray<ORCALine> lines, int numLines,
            int startLine, float radius, float2 currentResult, float2 optimalVelocityStart, float2 optimalVelocityDir,
            float optimalTLeft, float optimalTRight)
        {
        ...
        static LinearProgram2Output LinearProgram2DSegment(NativeArray<ORCALine> lines, int numLines, float radius,
            float2 optimalVelocityStart, float2 optimalVelocityDir, float optimalTLeft, float optimalTRight,
            float optimalT)
        {
            var hasIntersection = VectorMath.LineCircleIntersectionFactors(optimalVelocityStart, optimalVelocityDir,
                radius, out float resultTLeft, out float resultTRight);
            ...
            if (!hasIntersection)
            {
                ...
                return LinearProgram2DCollapsedSegment(lines, numLines, 0, radius, closestOnCircle,
                    optimalVelocityStart, optimalVelocityDir, optimalTLeft, optimalTRight);
            }
            ...
            for (int i = 0; i < numLines; i++)
            {
                ...
                var line = lines[i];
                var leftInfeasible = det(line.direction,
                    line.point - (optimalVelocityStart + optimalVelocityDir * resultTLeft)) > 0.0f;
                var rightInfeasible = det(line.direction,
                    line.point - (optimalVelocityStart + optimalVelocityDir * resultTRight)) > 0.0f;
                if (leftInfeasible || rightInfeasible)
                {
                    if (!ClipBoundary(lines, i, radius, out float tLeft, out float tRight))
                    {
                        ...
                        return new LinearProgram2Output
                        {
                            velocity = optimalVelocityStart +
                                       optimalVelocityDir * math.clamp(optimalT, resultTLeft, resultTRight),
                            firstFailedLineIndex = i,
                        };
                    }
                    ...
                    if (leftInfeasible && rightInfeasible)
                    {
                        if (math.abs(det(line.direction, optimalVelocityDir)) < 0.001f)
                        {
                            ...
                            var t1 = ClosestPointOnSegment(line.point, line.direction,
                                optimalVelocityStart + optimalVelocityDir * resultTLeft, tLeft, tRight);
                            var t2 = ClosestPointOnSegment(line.point, line.direction,
                                optimalVelocityStart + optimalVelocityDir * resultTRight, tLeft, tRight);
                            var t3 = ClosestPointOnSegment(line.point, line.direction,
                                optimalVelocityStart + optimalVelocityDir * optimalT, tLeft, tRight);
                        ...
                        else
                        {
                            ...
                            // The best point is now a single point, not a segment.
                            // So we can fall back to simpler code.
                            return LinearProgram2DCollapsedSegment(lines, numLines, i + 1, radius, result,
                                optimalVelocityStart, optimalVelocityDir, optimalTLeft, optimalTRight);
                        }
                    }
                    else
        ...
        static void LinearProgram3D(NativeArray<ORCALine> lines, int numLines, int numFixedLines, int beginLine,
            float radius, ref float2 result, NativeArray<ORCALine> scratchBuffer)
        {
            float distance = 0.0f;
            ...
            for (int i = beginLine; i < numLines; i++)
            {
                ...
                if (det(lines[i].direction, lines[i].point - result) > distance)
                {
                    ...
                    var lin = LinearProgram2D(projectedLines, numProjectedLines, radius,
                        new float2(-lines[i].direction.y, lines[i].direction.x), true);
