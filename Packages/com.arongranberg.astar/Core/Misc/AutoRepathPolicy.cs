    ...
    public class AutoRepathPolicy
    {
        ...
        public virtual bool ShouldRecalculatePath(Vector3 position, float radius, Vector3 destination, float time)
        {
            ...
            else
            {
                ...
                float squaredCost = (destination - lastDestination).sqrMagnitude /
                                    Mathf.Max((position - lastDestination).sqrMagnitude, radius * radius);
        ...
        public virtual void DidRecalculatePath(Vector3 destination, float time)
        {
            ...
            lastRepathTime -= (UnityEngine.Random.value - 0.5f) * JITTER_AMOUNT *
                              (mode == Mode.Dynamic ? maximumPeriod : period);
        }

        public void DrawGizmos(CommandBuilder draw, Vector3 position, float radius,
            Util.NativeMovementPlane movementPlane)
        {
            if (visualizeSensitivity && !float.IsPositiveInfinity(lastDestination.x))
            {
                float r = Mathf.Sqrt(Mathf.Max((position - lastDestination).sqrMagnitude, radius * radius) /
