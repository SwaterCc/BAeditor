...
namespace Sirenix.OdinInspector.Modules.Addressables.Editor
{
    public class CheckDuplicateBundleDependenciesValidator : GlobalValidator
    {
        ...
        public override IEnumerable RunValidation(ValidationResult result)
        {
            ...
            foreach (var kvp in dependencyGroupMap)
            {
                ...
                if (groups.Count > 1)
                {
                    ...
                    }, false).WithModifyRuleDataContextClick<CheckDuplicateBundleDependenciesValidator>("Ignore",
                        data => { data.IgnoredGUIDs.Add(dependencyGUID.ToString()); }).SetSelectionObject(
        ...
        {
            [EnumToggleButtons, HideLabel] public FixChoice FixChoice;
            ...
            [LabelWidth(120f)] [PropertySpace(10)] [ShowIf("FixChoice", FixChoice.Ignore, Animate = false)]
            public bool IgnoreForEveryone = true;

            [OnInspectorGUI]
            [PropertySpace(10)]
            [DetailedInfoBox("Note that duplicate assets may not always be an issue",
                "Note that duplicate assets may not always be an issue. If assets will never be requested by the same set of users (for example, region-specific assets), then duplicate dependencies may be desired, or at least inconsequential. Each Project is unique, so fixing duplicate asset dependencies should be evaluated on a case by case basis")]
            private void Dummy() { }
