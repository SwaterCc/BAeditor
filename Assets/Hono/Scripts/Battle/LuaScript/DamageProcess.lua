...
--local skillLevel = Actor.Logic:GetSkillLevel(SkillConfigid)
...
local FrontDamageProcess = function(attacker, target, damageInfo, damageConfig)        --【伤害计算前置流程，获取各项数值和预处理】
    ...
    damageArgs.Level_Final = attacker:GetAttrLua(60010)   
    damageArgs.ATK_Final = attacker:GetAttrLua(12100) * (math.random(9500, 10500)/10000) --攻击力
    ...
    if attacker:GetAttrLua(19) == 3  then
        damageArgs.DamageIsBad = true;
        else
        damageArgs.DamageIsBad = false;
    end
    ...
    damageArgs.DefRedFinal = DefRedFinal
    damageArgs.EleResFinal = EleResFinal
    damageArgs.DamRedFinal = DamRedFinal
...
DamageFormula = {
    ...
    NormalAttack = function(attacker, target, damageInfo, damageConfig, damageArgs, triggerParam)
        ...
        if triggerParam ~= 1 then
            logger.Info(
                    "<color=#F8B21B>DamageLog 元素:%s 最终伤害:<b>%s</b> 伤害者类型：%s %s AbilityID:%s  最终攻击力:%s * 技能倍率:%s * 附加增伤:%s * 乘区增伤:%s * 暴击倍率:%s * 最终承伤:%s</color>(防御力%s, 元素%s, 减免%s) , %s",
                    ...
                    damageArgs.CritDmgRate,
                    damageArgs.FinalRED,
                    damageArgs.DefRedFinal,
                    damageArgs.EleResFinal,
                    damageArgs.DamRedFinal,
    ...
    --按照百分比分摊伤害
    AttackSharePct = function(attacker, target, damageInfo, damageConfig, damageArgs)
        --跑一次普通伤害计算
        local finalDamageValue = DamageFormula["AttackTargetPct"](attacker, target, damageInfo, damageConfig, damageArgs)
        --根据命中人数数量分摊
        local finalDamageValue = math.max(1, finalDamageValue) / math.max(1, damageInfo.HitCount)
        logger.Info(
                "<color=#F8B21B>DamageLog 元素:%s 分摊后伤害:<b>%s</b>  分摊前伤害:%s  %s AbilityID:%s  技能倍率:%s / 命中人数:%s</color> , %s",
        ...
        return finalDamageValue + 1
    end,
    --固定值分摊伤害，无视所有减伤
    AttackConstantShare = function(attacker, target, damageInfo, damageConfig, damageArgs)
        --计算
        --注意：这个公式中 Skill_Per = 固定值而非比例了
        local finalDamageValue = damageArgs.Skill_Per * 10000 
        --根据命中人数数量分摊
        local finalDamageValue = math.max(1, finalDamageValue) / math.max(1, damageInfo.HitCount) 
        logger.Info(
                "<color=#F8B21B>DamageLog 元素:%s 分摊后伤害:<b>%s</b>  分摊前伤害:%s  %s AbilityID:%s  伤害绝对值:%s / 命中人数:%s</color> , %s",
                string.sub(tostring(damageConfig.ElementType), 1, string.find(tostring(damageConfig.ElementType), " ") - 2),
                math.floor(finalDamageValue),
                math.floor(finalDamageValue * math.max(1, damageInfo.HitCount)),
                logger.IntToStr(damageArgs.IsCritical and 1 or 0, "<color=#F8471B>暴击</color>"),
        ...
        --固定值分摊伤害，无视所有减伤
    AttackConstantWithSkillLevel = function(attacker, target, damageInfo, damageConfig, damageArgs)
        --计算
        --注意：这个公式中 Skill_Per = 固定值而非比例了
        local finalDamageValue = damageArgs.Skill_Per * 10000 * attacker.Logic:GetSkillLevel(damageInfo.SourceAbilityConfigId) / 10
