...
EAttrType = CS.Hono.Scripts.Battle.EAttrType
...
--==================================================================== 条件检测 ===================================================================
function CheckHitCount(attacker, target, damageInfo, params) --【判断伤害命中数量】
...
function CheckStat(attacker, target, damageInfo, params) --【属性绝对值对比】
...
function CheckEnemyType(attacker, target, damageInfo, params) -- 【敌人强度类型】
    -- addiParam[0] 目标的强度类型
...
function CheckDistance(attacker, target, damageInfo, params) --【判断距离】
    -- addiParam[0] 右值，比较距离。左值是实际距离
    -- addiParam[1] 比较符，1=大于，2=大于等于，3=等于，4=小于，5=小于等于
...
function CheckDamageSourceType(attacker, target, damageInfo, params) --【判断伤害来源】
...
function CheckTags(attacker, target, damageInfo, params) -- 【判断Tag】
...
function CheckBuffByLayer(attacker, target, damageInfo, addiParam) --【判断BUFF】
    -- addiParam[0] 对象，0=攻击者，1=目标
...
local FrontDamageProcess = function(attacker, target, damageInfo, damageConfig)       --【伤害计算前置流程，获取各项数值和预处理】
    ...
    damageArgs.Skill_Per = damageInfo.Skill_Per / 10000                               --技能倍率
    ...
    damageArgs.ElementPenPCT = attacker:GetAttrLuaByType(EAttrType.AttrElementPenPCT) --全属性抗性穿透
    ...
    --抗性减伤计算
    local EleResFinal = ElementDamage(attacker, target, damageInfo, damageConfig)
    --伤害减免计算
    local DamRedFinal = 1
    local AllDamRed = {
...
function ElementDamage(attacker, target, damageInfo, damageConfig)
...
DamageFormula = {                  
    ...
    Heal = function(attacker, target, damageInfo, damageConfig, damageArgs)                            
        ...
        local healAttack = attacker:GetAttrLuaByType(EAttrType.AttrAttack)    --DEMO2 治疗强度 = 攻击力
        local healUp = attacker:GetAttrLuaByType(EAttrType.AttrHeal) / 10000
        local beHealed = target:GetAttrLuaByType(EAttrType.AttrHealed) / 10000
    ...
    AttackBuffDot = function(attacker, target, damageInfo, damageConfig, damageArgs)                     
        ...
        damageArgs.CritDmgRate = 1
        --获取Buff层数
        --local dotCount = target:GetBuffLayer(damageConfig.SourceId) or 1 
