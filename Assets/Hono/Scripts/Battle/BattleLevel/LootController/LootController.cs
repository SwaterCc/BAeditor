...
using Hono.Scripts.Battle.BattleUI.RougePanel;
	...
	public class LootController {
		...
		public LootController() {
			...
			_default.LootSettings.Add(new LootSetting() {
				...
				attrTypeType = EAttrType.AttrAttackAdd,
				ChangeAttrValue = 40
			});
			_default.LootSettings.Add(new LootSetting() {
				LootFunctionType = ELootFunctionType.AttrChange,
				attrTypeType = EAttrType.AttrMaxHpPer,
				ChangeAttrValue = 2000
			});
			_default.LootSettings.Add(new LootSetting() {
				LootFunctionType = ELootFunctionType.AttrChange,
				attrTypeType = EAttrType.AttrCritAdd,
				ChangeAttrValue = 1000
			});
			_default.LootSettings.Add(new LootSetting() {
				LootFunctionType = ELootFunctionType.AttrChange,
				attrTypeType = EAttrType.AttrCritDamageAdd,
				ChangeAttrValue = 1000
			});
			_default.LootSettings.Add(new LootSetting() {
				LootFunctionType = ELootFunctionType.AttrChange,
				attrTypeType = EAttrType.AttrSkillCDPCTAdd,
				ChangeAttrValue = 1000
			});
			_default.LootSettings.Add(new LootSetting() {
				LootFunctionType = ELootFunctionType.AttrChange,
				attrTypeType = EAttrType.AttrMoveSpeedPCTAdd,
				ChangeAttrValue = 1000
			});
			_default.LootSettings.Add(new LootSetting() {
		...
		public void CreateLoot(Vector3 pos) {
			/*ActorManager.Instance.CreateLoot((loot) => {
				loot.SetAttr(ELogicAttr.AttrPosition, pos, false);
			});*/
		}
		...
		public void CreateRougeCards(int uid) {
			...
			RougePanel.Instance.PickUpUid = uid;
			RougePanel.Instance.Show();
		}
