...
namespace Hono.Scripts.Battle.Core
{
    ...
    public class Bullet : Unit, IGPoolObject
    {
            ...
            int hitNotTargetDamageId)
        {
            ...
            _hitTargetDamageId = hitTargetDamageId;
            _hitNotTargetDamageId = _hitNotTargetDamageId;
            UnitTransform.Forward = Quaternion.AngleAxis(yAxisAngle, Vector3.up) * Vector3.forward;
        ...
        private void move(float dt)
        {
            switch (_bulletType)
            {
                case EBulletType.LockTargetBullet:
                    ...
                    //获取移动方向
                    UnitTransform.Forward = Quaternion.AngleAxis(realRotAngle, Vector3.up) * UnitTransform.Forward;
                    ...
                    {
                        //和目标的距离小于半径和
                        SingleHit(_target, EDamageSourceType.Bullet, _bulletData.id, false, _hitTargetDamageId);
                    }
        ...
        private void checkCollision()
        {
            if (CommonUtility.HitRayCast(_checkBox, UnitTransform.Pos, UnitTransform.Rot, ref _maxHitResul))
            {
                foreach (var uid in _maxHitResul)
                {
                    ...
                    SingleHit(target, EDamageSourceType.Bullet, _bulletData.id, false, _hitNotTargetDamageId);
