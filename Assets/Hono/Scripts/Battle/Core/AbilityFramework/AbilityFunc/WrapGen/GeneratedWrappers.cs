...
namespace Hono.Scripts.Battle.AbilityFramework
{
    public partial class Ability
    {
        public static partial class AbilityEnv
        {
            ...
            private class __Gen_AFuncWrap_AddVFXByKey : IReturnInt
            {
                public int CallFunc(Ability caller, ANode node, List<AParams> @params)
                {
                    var p0 = node.ParseInt(@params[0]);
                    var p1 = (String)node.ParseRef(@params[1]);
                    var p2 = (VFXSetting)node.ParseRef(@params[2]);
                    return caller._functionDefine.AddVFXByKey(p0, p1, p2);
                }
            }


            private class __Gen_AFuncWrap_AddVFXByPath : IReturnInt
            {
                public int CallFunc(Ability caller, ANode node, List<AParams> @params)
                {
                    var p0 = node.ParseInt(@params[0]);
                    var p1 = (String)node.ParseRef(@params[1]);
                    var p2 = (VFXSetting)node.ParseRef(@params[2]);
                    return caller._functionDefine.AddVFXByPath(p0, p1, p2);
                }
            }
