...
namespace Hono.Scripts.Battle.AbilityFramework {
	public partial class AFunctionDefine {
		...
		public void SkillRTModify(int skillId, ESkillModifyType modifyType, int p1, int p2) {
			...
			if (combatComp.TryGetSkillModifier(getSkillId(skillId), out SkillModifier modifier)) {
		...
		public void LessSkillCD(int skillId, float lessValue) {
			...
			if (combatComp.TryGetSkill(getSkillId(skillId), out var skill)) {
		...
		public void CreateLockTargetBullet(
			...
			bullet.UnitTransform.Pos += attacker.UnitTransform.Rot * offset;;
		...
		[AbilityFunction("VFX")]
		public int AddVFXToWorldByKey(int creatorUid, string key, float vfxDuration, Vector3 position, Vector3 rot) {
			if (!tryGetUnit(creatorUid, out var creator)) {
				return -1;
			}

			var path = creator.GetVFXPathByKey(key);
		...
		[AbilityFunction("VFX")]
		public int AddVFXToWorldByPath(string path, float vfxDuration, Vector3 position, Vector3 rot) {
			if (string.IsNullOrEmpty(path)) {
				return -1;
			}
		...
		[AbilityFunction("VFX")]
		public int AddVFXToUnitByKey(int creatorUid, int targetUid, string key, float duration, string bindBond, Vector3 position, Vector3 eulerAngle) {
			if (!tryGetUnit(creatorUid, out var creator)) {
				return -1;
			}
		...
		[AbilityFunction("VFX")]
		public int AddVFXToUnitByPath(int targetUid, string path, float duration, string bindBond, Vector3 position, Vector3 eulerAngle) {
			if (!tryGetUnit(targetUid, out var target)) {
				return -1;
			}
		...
		[AbilityFunction]
		public bool UnitHasTag(int unitUid,int tag,bool strict) {
			if(!tryGetUnit(unitUid,out Unit unit)) {
				return false;
			}

			return unit.Tags.HasTag(tag, strict);
		}
