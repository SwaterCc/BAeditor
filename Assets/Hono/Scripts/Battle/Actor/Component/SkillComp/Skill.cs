    ...
    public partial class ActorLogic
    {
        public class Skill : IAPoolObject
        {
                    ...
                    foreach (var resItem in resItems.Items)
                    {
                        switch (resItem.ResourceType)
                        {
                            case EBattleResourceType.Energy:
                                var mp = Logic.Self.GetAttr((EAttrType)resItem.ResId);
                ...
                foreach (var resItems in SkillData.skillResCost)
                {
                    ...
                    foreach (var resItem in resItems.Items)
                    {
                        ...
                        switch (resItem.ResourceType)
                        {
                            case EBattleResourceType.Energy:
                                var mp = Logic.Self.GetAttr((EAttrType)resItem.ResId);
                                Logic.Self.SetAttr((EAttrType)resItem.ResId, mp - resItem.Value, false);
                                break;
                        }
                    }
                }
            ...
            public void OnTick(float dt)
            {
                ...
                if (_curCdPercent > 0)
                {
                    var finalCdPct = (Logic.Self.GetAttr(EAttrType.AttrSkillCDPCT) / 10000f + 1);
                ...
                //更新攻速
                var attackSpeed = Logic.Self.GetAttr(EAttrType.AttrAttackSpeedPCT) / 10000f + 1;
