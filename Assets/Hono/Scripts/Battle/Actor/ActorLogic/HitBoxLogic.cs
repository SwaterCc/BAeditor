...
namespace Hono.Scripts.Battle
{
	...
	public class HitBoxLogic : ActorLogic
    {
        ...
        private void aoeHit(Actor attacker, DamageInfo damageInfo)
        {
            ...
            targetIds.AddRange(ActorManager.Instance.UseFilter(Actor, _filterSetting));
            ...
            foreach (var beHurtComp in finalTargets)
            {
                ...
                BattleEventManager.Instance.TriggerActorEvent(_sourceActorId, EBattleEventType.OnHitDamage,
                    hitDamageInfo);
                beHurtComp.OnBeHurt(hitDamageInfo);
            }
        }
        ...
        {
            if (ActorManager.Instance.GetActorRtState(_sourceActorId) == EActorRunningState.NotExist &&
                !Actor.IsExpired)
            {
                ActorManager.Instance.RemoveActor(Uid);
                return;
