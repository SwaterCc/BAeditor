...
namespace Hono.Scripts.Battle.Tools.DebugTools {
	...
	public class AbilityDebugInspector : MonoBehaviour {
		[ShowInInspector]
		public DamageItem DamageConfig;
		...
		public void Start() {
			_attacker = ActorManager.Instance.CreateActor(EActorType.Pawn, 1);
			_target = ActorManager.Instance.CreateActor(EActorType.Pawn, 2);
		}
		...
		public void HitTest() {
			...
			_attacker.ActorLogic.SetAttr(EAttrType.Attack, 500, false);
			_attacker.ActorLogic.SetAttr(EAttrType.Hp, 100, false);
			_attacker.ActorLogic.SetAttr(EAttrType.Mp, 100, false);
