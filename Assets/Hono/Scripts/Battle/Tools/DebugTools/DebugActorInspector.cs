...
namespace Hono.Scripts.Battle.Tools.DebugTools
{
    public class DebugActorInspector : MonoBehaviour
    {
        ...
        [Button("给自己加属性")]
        public void SetAttr(EAttrType attrId, int value)
        {
            _actor.SetAttr(attrId, _actor.GetAttr(attrId) + value, false);
        }
        ...
        private void Update()
        {
            ...
            if (Enum.IsDefined(typeof(EAttrType), ShowAttrId))
            {
                ShowAttrText = $"{(EAttrType)ShowAttrId} : " + _actor.GetAttr((EAttrType)ShowAttrId);
            }
