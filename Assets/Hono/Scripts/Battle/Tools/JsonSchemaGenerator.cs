...
namespace Hono.Scripts.Battle.Tools
{
    public static class JsonSchemaGenerator
    {
        ...
        private static void GenerateJsonSchema(string outputDir, IEnumerable<Type> componentTypes)
        {
            ...
            File.WriteAllText(jsonFilePath, Newtonsoft.Json.JsonConvert.SerializeObject(jsonSchema, Newtonsoft.Json.Formatting.Indented));
...
{{
    ...
    {{
        ...
        {{
            ...
            {{
{string.Join(",\n", fields.Select(f => $"                {f.Name} = token[\"{f.Name}\"]{(IsValueType(f.FieldType) ? ".Value" : "?")}.To{GetTypeName(f.FieldType)}(),"))}
            }};
        }}
    }}
}}
";
        ...
        /// 生成 ComponentFactories 和 ParamParsers 静态字段
        /// </summary>
        private static void GenerateComponentFactoriesAndParamParsers(string outputDir, IEnumerable<Type> componentTypes)
        {
            ...
            {
                var componentName = type.Name;
                factoriesContent.Add($"            {{ EUnitComponentKey.{componentName}, new UnitComponentFactory<{type.Name}>() }},");
                parsersContent.Add($"            {{ EUnitComponentKey.{componentName}, new {componentName}ParamParser() }},");
            }
...
";
        ...
        /// 生成 ComponentCtorParams 类和 Ctor 方法
        /// </summary>
        private static void GenerateComponentCtorParamsAndCtorMethods(string outputDir, IEnumerable<Type> componentTypes)
        {
            foreach (var type in componentTypes)
            {
                var componentName = type.Name;
