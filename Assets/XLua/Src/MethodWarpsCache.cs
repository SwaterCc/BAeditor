...
namespace XLua
{
    public class OverloadMethodWrap
    {
            ...
            for (int i = 0; i < paramInfos.Length; i++)
            {
                ...
                else
                {
                    ...
                    var paramType =
                        ...
                        (!paramInfos[i].ParameterType.IsArray && paramInfos[i].ParameterType.IsByRef)
                    ...
                    if (paramInfos[i].IsOptional)
                    {
                        ...
                        {
                            defalutValue = defalutValue.GetType() == typeof(Missing)
        ...
        public bool Check(RealStatePtr L)
        {
            ...
            return paramsType != null
                ? (luaStackPos < luaTop + 1 ? checkArray[checkArray.Length - 1](L, luaStackPos) : true)
        ...
        public int Call(RealStatePtr L)
        {
            try
            {
                ...
                if (method.IsDefined(typeof(ObsoleteAttribute), true))
                {
                    ...
                    UnityEngine.Debug.LogWarning("Obsolete Method [" + method.DeclaringType.ToString() + "." +
                                                 method.Name + "]: " + info.Message);
                }
#endif
                object target = null;
    ...
    public class MethodWrapsCache
    {
        ...
        public LuaCSFunction GetDelegateWrap(Type type)
        {
            ...
            {
                delegateCache[type] =
                    _GenMethodWrap(type, type.ToString(), new MethodBase[] { type.GetMethod("Invoke") }).Call;
            }
                    ...
                    methodsOfType[eventName] = (L) =>
                    {
                        ...
                        try
                        {
                            object handlerDelegate =
                                translator.CreateDelegateBridge(L, eventInfo.EventHandlerType, start_idx + 2);
                            if (handlerDelegate == null)
                            {
                                return LuaAPI.luaL_error(L,
                                    "invalid #" + (start_idx + 2) + ", needed:" + eventInfo.EventHandlerType);
                            }
